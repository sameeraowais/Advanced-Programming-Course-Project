...
public class FXMLMonthController {
    Session session;
     User user1;
    ...
    @FXML
    void m_w1(ActionEvent event) throws IOException {
       Window owner = m_w1.getScene().getWindow();
...
personal_info_stage.show();
    ...
    @FXML
    void m_w2(ActionEvent event) throws IOException {
        Window owner = m_w2.getScene().getWindow();
        if(user1.getWeeks()==null){
        showAlert(Alert.AlertType.ERROR, owner, "Form Error!", "you need to do week 1 First");  
                 return;    
        }
        else if(user1.getWeeks().equalsIgnoreCase("m_w1")&&user1.getDayscompleted()>=7){
        user1.setWeeks("m_w2");
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
...
personal_info_stage.show();
         ...
         showAlert(Alert.AlertType.INFORMATION, owner, "Form Carethy", "this week challenge is already done ");  
                 return; 
        }else if(user1.getDayscompleted()<14 && user1.getWeeks().equalsIgnoreCase("m_w2")){
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
        tx.commit();
...
personal_info_stage.show();
    ...
    @FXML
    void m_w3(ActionEvent event) throws IOException {
    Window owner = m_w3.getScene().getWindow();
       if(user1.getWeeks()==null){
        showAlert(Alert.AlertType.ERROR, owner, "Form Error!", "you need to do week 1 First");  
                 return;    
        }
        else if(user1.getWeeks().equalsIgnoreCase("m_w2")&& user1.getDayscompleted()>=14){
        user1.setWeeks("m_w3");
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
...
Scene personal_info_scene = new Scene(root);
Stage personal_info_stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
personal_info_stage.setTitle("week 3 challenge");
personal_info_stage.setScene(personal_info_scene);
personal_info_stage.show();
        }
       
        else if ( user1.getDayscompleted()>=21){
         showAlert(Alert.AlertType.INFORMATION, owner, "Form Carethy", "this week challenge is already done ");  
                 return; 
        }
          else if(user1.getDayscompleted()<21 && user1.getWeeks().equalsIgnoreCase("m_w3")){
            Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
        tx.commit();
...
personal_info_stage.show(); 
    ...
    @FXML
    void m_w4(ActionEvent event) throws IOException {
    Window owner = m_w4.getScene().getWindow();
       if(user1.getWeeks()==null){
        showAlert(Alert.AlertType.ERROR, owner, "Form Error!", "you need to do week 1 First");  
                 return;    
        }
        else if(user1.getWeeks().equalsIgnoreCase("m_w3")&& user1.getDayscompleted()>=21){
        user1.setWeeks("m_w4");
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
...
Scene personal_info_scene = new Scene(root);
Stage personal_info_stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
personal_info_stage.setTitle("week 4 challenge");
personal_info_stage.setScene(personal_info_scene);
personal_info_stage.show();
        }
       
        else if (user1.getWeeks().equalsIgnoreCase("m_w4")&&user1.getDayscompleted()>=28){
         showAlert(Alert.AlertType.INFORMATION, owner, "Form Carethy", "this week challenge is already done ");  
                 return; 
        }
        else if (user1.getDayscompleted()<28 && user1.getWeeks().equalsIgnoreCase("m_w4")){
           Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
        tx.commit();
