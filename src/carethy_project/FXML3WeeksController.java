...
public class FXML3WeeksController  {
    ...
    private Button w3_3;
    @FXML
    private void w3_1(ActionEvent event) throws IOException {
        Window owner = w3_1.getScene().getWindow();
...
personal_info_stage.show();
    ...
    @FXML
    private void w3_2(ActionEvent event) throws IOException {
        Window owner = w3_2.getScene().getWindow();
         if(user1.getWeeks()==null){
        showAlert(Alert.AlertType.ERROR, owner, "Form Error!", "you need to do week 1 First");  
                 return;    
        }
        else if(user1.getWeeks().equalsIgnoreCase("w3_1")&& user1.getDayscompleted()>=7){
        user1.setWeeks("w3_2");
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
...
personal_info_stage.show();
        ...
        else if(user1.getDayscompleted()<14 && user1.getWeeks().equalsIgnoreCase("w3_2")){
           Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
        tx.commit();
...
personal_info_stage.show();
    ...
    @FXML
    private void w3_3(ActionEvent event) throws IOException {
        Window owner = w3_3.getScene().getWindow();
         if(user1.getWeeks()==null){
        showAlert(Alert.AlertType.ERROR, owner, "Form Error!", "you need to do week 2 First");  
                 return;    
        }
        else if(user1.getWeeks().equalsIgnoreCase("w3_2")&& user1.getDayscompleted()>=14){
        user1.setWeeks("w3_3");
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
...
personal_info_stage.show();
         ...
         showAlert(Alert.AlertType.INFORMATION, owner, "Form Carethya", "this week challenge is already done ");  
                 return; 
        }
        else if(user1.getDayscompleted()<21 && user1.getWeeks().equalsIgnoreCase("w3_3")){
            Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.update(user1);
        tx.commit();
